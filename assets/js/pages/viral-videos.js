/* =========================================================
   页面 2：爆款抖音 / 小红书视频（拆为三个一级导航）
   - viral-videos（爆款视频）：实时热榜 + 采集榜 + 二级类目筛选 + 列表
   - vv-analyzer（视频智能解析）：猫咪账号专属 · 发链接一键解析
   - vv-library（素材库）：我的收藏 / 手动新增 / 批量 / 导出
...
function genSample(count) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    items.push({
      ...
      // 示例榜无真实视频直链，url 指向平台首页根；点击「查看原视频」会改走“搜原视频”定位具体视频
...
function updateItem(id, fn) {
  ...
  S.saveVideos(v); App.renderCurrent();
}
...
function removeItem(id) {
  ...
  S.saveVideos(v); U.toast('已删除'); App.renderCurrent();
}
...
/* ===================== 修复：查看原视频（不再死跳首页） ===================== */
function openOriginal(it) {
  var u = it.url || '';
  var homeRoots = ['https://www.douyin.com/', 'https://www.xiaohongshu.com/'];
  if (u && homeRoots.indexOf(u) === -1) { window.open(u, '_blank'); return; }
  // 示例素材只有平台首页根，没有具体视频直链 → 跳平台搜索该标题，定位原视频 / 同款
  var q = encodeURIComponent(it.title || '');
  var searchUrl = it.platform === 'xhs'
...
/* ===================== 共享：二级类目导航 ===================== */
function renderTracks(state, groupBar, chips) {
  U.clear(groupBar);
  var groups = [{ id: 'all', name: '全部' }, { id: 'cat', name: '🐱 猫咪' }, { id: 'dog', name: '🐶 狗狗' }, { id: 'exotic', name: '🐉 异宠' }, { id: 'general', name: '📂 综合' }];
  groups.forEach(function (g) {
    groupBar.appendChild(U.el('span', { class: 'tag' + (state.catGroup === g.id ? ' active' : ''), text: g.name, onclick: function () { state.catGroup = g.id; state.track = 'all'; App.renderCurrent(); } }));
  });
  U.clear(chips);
  var subs = state.catGroup === 'all' ? TRACKS : CATS.filter(function (c) { return c.id === state.catGroup; })[0].tracks;
  subs.forEach(function (t) {
    chips.appendChild(U.el('span', { class: 'tag sub' + (state.track === t ? ' active' : ''), text: t, onclick: function () { state.track = t; App.renderCurrent(); } }));
  });
}

/* ===================== 共享：实时热榜（真实联网） ===================== */
var liveType = 'douyin';
function loadLive(box, srcEl) {
  srcEl.innerHTML = '<span class="live-dot wait"></span> 抓取中…';
  U.clear(box); box.appendChild(U.el('div', { class: 'muted', text: '联网抓取中…' }));
  U.fetchTrending(liveType, 9000).then(function (res) {
    if (!res.ok || !res.items.length) {
      srcEl.innerHTML = '<span class="live-dot off"></span> 暂未获取';
      U.clear(box);
      box.appendChild(U.el('div', { class: 'empty', text: '实时热榜获取失败。若部署在 github.io 受跨域限制，建议部署到 Cloudflare Pages 启用服务端联网；或稍后刷新。' }));
      return;
    }
    srcEl.innerHTML = '<span class="live-dot on"></span> 实时 · ' + res.items.length + ' 条';
    U.clear(box);
...
function renderAnalysis(container, p, input) {
  U.clear(container);
...
function saveParsedAsMaterial(p, input) {
  ...
  v.manual.unshift({
    ...
    // 保存的是用户粘贴的真实视频链接，查看原视频时能直接打开具体视频
...
function renderDoubaoPrompt(container, link, title, desc) {
  U.clear(container);
...
function openManual() {
  ...
  var urlI = U.el('input', { class: 'input', placeholder: '外部视频链接（填了才能「查看原视频」直达具体视频）' });
  ...
  U.modal({ title: '手动新增爆款素材', body: body, actions: [{ label: '取消' }, { label: '保存', primary: true, onClick: function () {
    ...
    v.manual.unshift({
      ...
      // 手动新增若填了链接 → 保存真实链接；否则回退平台首页根（查看原视频会改走“搜原视频”）
    ...
    S.saveVideos(v); U.toast('已保存手动素材'); App.renderCurrent();
  } }] });
}
function field(label, node) { var d = U.el('div', { class: 'field' }); d.appendChild(U.el('label', { text: label })); d.appendChild(node); return d; }

function toggleBatch(state) { state.batchMode = !state.batchMode; state.selected = {}; App.renderCurrent(); }
function batchArchive(state) {
  ...
  S.saveVideos(v); U.toast('已归档 ' + ids.length + ' 条'); state.batchMode = false; state.selected = {}; App.renderCurrent();
}
function batchDelete(state) {
  var ids = Object.keys(state.selected); if (!ids.length) return U.toast('请先勾选');
  U.modal({ title: '批量删除', body: '确定删除选中的 ' + ids.length + ' 条素材？', actions: [{ label: '取消' }, { label: '删除', primary: true, onClick: function () {
    ...
    S.saveVideos(v); U.toast('已删除'); state.batchMode = false; state.selected = {}; App.renderCurrent();
  } }] });
}
...
App.pages['vv-analyzer'] = function (root) {
  U.clear(root);
  var state = { link: '', title: '', desc: '' };
  ...
  function finishAnalyze(link, title, desc) {
    ...
    renderAnalysis(aResult, parseVideo({ link: link, title: title, desc: desc }), { link: link, title: title, desc: desc });
    renderDoubaoPrompt(aDoubao, link, title, desc);
  }
  function prepareDoubaoPrompt() {
    ...
    if (link && !title) {
      ...
      fetchVideoMeta(link).then(function (meta) {
        ...
        renderDoubaoPrompt(aDoubao, link, title, desc);
      }).catch(function () { renderDoubaoPrompt(aDoubao, link, title, desc); });
    } else {
      renderDoubaoPrompt(aDoubao, link, title, desc);
    }
  }
